...
const Communication = () => {
  ...
  useEffect(() => {
    const tab = location.pathname.includes("/announcements")
      ? "announcement"
      : "compose";
    setActiveTab(tab);
  }, [location.pathname]);
  ...
  const fetchEvents = async () => {
    try {
      ...
      console.log(
        "🔍 [Communication] Organizer ID from localStorage:",
        organizerId,
      );

      if (!organizerId) {
        console.error(
          "❌ [Communication] No organizer ID found in localStorage!",
        );
      ...
      if (response.data.success) {
        ...
        response.data.data.forEach((event, index) => {
          console.log(
            `📋 [Communication] Event ${index + 1}: ${event.title || event.name}, Organizer: ${event.organizer}`,
          );
        });
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        alert(
          `Email sent successfully to ${response.data.data.recipientCount || 0} participants!`,
        );
        setEmailData({
          subject: "",
          message: "",
      ...
      alert(
        error.message ||
          "Failed to send email. Please check if there are participants matching your filter.",
      );
    } finally {
      setSending(false);
    }
  ...
  const statusBadge = (status) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-lg text-xs font-medium ${colors[status] || colors.PENDING}`}
      >
        {status}
      </span>
    );
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 p-6 space-y-8">
        ...
        {emailConfigStatus && (
          <div
            className={`rounded-2xl p-6 flex items-start gap-4 shadow-lg ${
          ...
          >
            ...
            >
              <Settings
                size={20}
                strokeWidth={2.5}
                className={`${checkingConfig ? "animate-spin" : "group-hover:rotate-90 transition-transform"}`}
              />
            </button>
          </div>
        )}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-zinc-400 font-semibold mt-2">
                Send emails and announcements to participants
              </p>
            </div>
          </div>
          <div className="flex gap-4 items-center">
            <button
              ...
              className="group px-4 py-2 border border-gray-300 dark:border-white/10 rounded-xl hover:bg-[#B9FF66]/10 hover:border-[#B9FF66] transition-all font-bold text-gray-700 dark:text-zinc-300"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="group bg-white/80 dark:bg-white/[0.03] backdrop-blur-sm rounded-2xl p-6 shadow-lg dark:shadow-none border border-white/60 dark:border-white/5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between">
              ...
              <div className="p-4 bg-[#B9FF66]/20 rounded-2xl shadow-lg group-hover:scale-110 transition-transform">
                <Users size={24} className="text-[#191A23]" strokeWidth={2.5} />
              </div>
            </div>
            <div className="absolute top-2 right-2 w-2 h-2 bg-[#B9FF66] rounded-full opacity-60"></div>
          </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const template = displayTemplates.find(
                        ...
                        className="w-full px-6 py-3 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#B9FF66] focus:border-[#B9FF66] bg-white/80 dark:bg-white/5 backdrop-blur-sm font-bold text-gray-900 dark:text-white"
                      >
                          ...
                          <button
                            ...
                            className={`group flex items-center gap-3 p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${
                              emailData.recipientFilter === filter.value
                                ? "border-[#191A23] bg-[#B9FF66]/10 text-[#191A23] dark:text-white shadow-lg"
                                : "border-gray-200 dark:border-white/10 text-gray-600 dark:text-zinc-400 hover:border-[#B9FF66]"
                            }`}
                          >
                            <filter.icon
                              size={16}
                            ...
                            {emailData.recipientFilter === filter.value && (
                              <div className="ml-auto w-2 h-2 bg-[#191A23] rounded-full"></div>
                            )}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-6 py-4 border border-gray-200 dark:border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#B9FF66] focus:border-[#B9FF66] bg-white/80 dark:bg-white/5 backdrop-blur-sm font-semibold text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-zinc-500"
                      />
                    </div>
...
{{eventDate}} - Event date"
                        className="w-full px-6 py-4 border border-gray-200 dark:border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#B9FF66] focus:border-[#B9FF66] bg-white/80 dark:bg-white/5 backdrop-blur-sm font-mono text-sm text-gray-900 dark:text-white resize-none"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                      <p className="text-sm text-gray-600 dark:text-zinc-400 font-semibold">
                        This email will be sent to participants matching the
                        selected filter.
                      </p>
                      <button
                        ...
                        className="group flex items-center gap-3 px-8 py-3 bg-[#191A23] text-[#B9FF66] rounded-2xl hover:shadow-xl hover:scale-105 transition-all font-bold disabled:opacity-50 disabled:hover:scale-100"
                      >
                        {sending ? (
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <div className="flex gap-3">
                        {["INFO", "WARNING", "URGENT", "ANNOUNCEMENT"].map(
                          (type) => (
                            <button
                              key={type}
                              onClick={() =>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#B9FF66] bg-white dark:bg-white/5 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-zinc-500"
                      />
                    </div>
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5 max-h-[500px] overflow-y-auto">
              {displayHistory.length === 0 ? (
                <div className="p-8 text-center">
                  <div className="w-24 h-24 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-white/5 dark:to-white/10 rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-lg dark:shadow-none">
                    <Mail
                      size={32}
                ...
                displayHistory.map((item) => (
                  <div
                    key={item._id}
                    className="p-5 hover:bg-[#B9FF66]/5 dark:hover:bg-white/[0.02] transition-all cursor-pointer"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h4 className="font-black text-gray-900 dark:text-white text-base line-clamp-1">
                        {item.subject}
                      </h4>
                      {statusBadge(item.status)}
                    </div>
                    <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-zinc-400 font-semibold">
                    ...
                    <span className="inline-block mt-3 px-3 py-1.5 bg-[#B9FF66]/20 text-[#191A23] rounded-xl text-sm font-bold border border-[#B9FF66]/30">
